package org.petuum.jbosen_yarn.client;
...
import org.petuum.jbosen_yarn.common.Log4jPropertyHelper;
import org.petuum.jbosen_yarn.common.PsYarnConstants;
import org.petuum.jbosen_yarn.common.YarnUtil;
...
public class Client {
    ...
    // Ps application
    private String psAppJarLocalPath;
    private String psAppOtherArgs;

    private static final String appMasterJarPath = "AppMaster.jar";
    // Hardcoded path to custom log_properties
    private static final String log4jPath = "log4j.properties";

    private String psHDFSPathPrefix = "";
    /**
     * @param args
   ...
   */
    public Client(Configuration conf) throws Exception {
        this("org.petuum.jbosen_yarn.application_master.ApplicationMaster", conf);
    }
    ...
    Client(String appMasterMainClass, Configuration conf) {
        ...
        opts.addOption("timeout", true,
                "Application timeout in milliseconds. Default 600000");
        opts.addOption(
                "master_memory",
                true,
        ...
        opts.addOption("app_master_jar", true,
                "Jar file containing the application master");
        opts.addOption("launch_script_path", true, "User's launch script path");
        opts.addOption("ps_app_jar_local_path", true,
                "The class path of the running PS App");
        opts.addOption("ps_app_other_args", true,
                "The additional args needed for the PS App");
        opts.addOption("ps_hdfs_dir_prefixs", true,
                "The dir path on HDFS that all files should prefix by");
   ...
   */
    ...
    public boolean init(String[] args) throws ParseException {
        ...
        appName = cliParser.getOptionValue("appname", "Petuum");
        startPort = Integer.parseInt(cliParser.getOptionValue("start_port",
                "9999"));
        amPriority = Integer.parseInt(cliParser
                .getOptionValue("priority", "10"));
        ...
        if (!cliParser.hasOption("app_master_jar")) {
            throw new IllegalArgumentException(
                    "No jar file specified for application master");
        }

        appMasterJar = cliParser.getOptionValue("app_master_jar");

        scriptPath = cliParser.getOptionValue("launch_script_path");
        psAppJarLocalPath = cliParser.getOptionValue("ps_app_jar_local_path",
                "");
        psAppOtherArgs = cliParser.getOptionValue("ps_app_other_args", "");
        psHDFSPathPrefix = cliParser.getOptionValue("ps_hdfs_dir_prefixs", "");
        ...
        if (containerMemory < 0 || containerVirtualCores < 0) {
            throw new IllegalArgumentException(
                    "Invalid container memory/vcores specified," + " exiting."
    ...
    public boolean run() throws IOException, YarnException {
        ...
        YarnUtil.copyAndAddToLocalResources(fs, appMasterJar, appMasterJarPath, psHDFSPathPrefix, localResources, null);
        

        // Set the log4j properties if needed
        if (!log4jPropFile.isEmpty()) {
            YarnUtil.copyAndAddToLocalResources(fs, log4jPropFile, log4jPath, psHDFSPathPrefix, localResources, null);
        }

        LOG.info("Copy all other files needed for the app from local file system");
        String psAppName = new Path(psAppJarLocalPath).getName();
        String psAppJarHDFSPath = YarnUtil.copyToHDFS(fs, psAppJarLocalPath, psHDFSPathPrefix, localResources, null);
        ...
        // Passing the yarn app related variabes through enviroment. This is
        // cleaner than command line.
        env.put(PsYarnConstants.PS_APP_HDFS_PATH, psAppJarHDFSPath);
        env.put(PsYarnConstants.PS_APP_NAME, psAppName);
        env.put(PsYarnConstants.PS_APP_ARGS, psAppOtherArgs);
        env.put(PsYarnConstants.PS_HDFS_PREFIX, psHDFSPathPrefix);
        ...
        vargs.add(Environment.JAVA_HOME.$$() + "/bin/java -ea");
        ...
        // vargs.add("--server_file_path " + String.valueOf(serverFilePath));
    ...
    private boolean monitorApplication(ApplicationId appId)
        ...
        while (true) {
            ...
            // ML program usually work long time, so the timeout should be very
            // long.
            // For convenient purpose, just comment this part and never force
            // kill app.
            // if (System.currentTimeMillis() > (clientStartTime +
            // clientTimeout)) {
