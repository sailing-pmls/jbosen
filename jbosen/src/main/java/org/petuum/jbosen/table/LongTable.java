...
import org.petuum.jbosen.row.long_.LongRow;
import org.petuum.jbosen.row.long_.LongRowUpdate;
import org.petuum.jbosen.row.long_.SparseLongRowUpdate;

/**
 * An object of this class offers a view of a table that contains rows of
 * long values. Methods of this class should only be accessed from registered
 ...
 * {@link PsTableGroup#createDenseLongTable(int, int, int)} or
 * {@link PsTableGroup#createSparseLongTable(int, int)}, and be retrieved
 * with {@link PsTableGroup#getLongTable(int)}.
 */
public final class LongTable {
    ...
    public <RowType extends LongRow> RowType get(int rowId) {
     ...
     */
    public long get(int rowId, int columnId) {
        LongRow longRow = (LongRow) clientTable.get(rowId).getRow();
        return longRow.get(columnId);
    }
     ...
     */
    public void inc(int rowId, LongRowUpdate rowUpdate) {
     ...
     */
    public void inc(int rowId, int columnId, long update) {
